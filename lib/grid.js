function Grid($grid) {
	...
	this.init = function () {
		...
		$grid.$container.bind('click dblclick', that, mouseOverCellHandler);
		...
		// let's detect paste events
		$grid.$container.parent().bind("paste", function(e) {
		    alert(e.clipboardData); // [object Clipboard]
		});

		
		// trigger a click handler on the first row first column by default
		that.$grid.$container.children().eq(0).children().eq(0).children().click();
	...
	function generateRows() {
		...
		for (row = 1; row <= nb_rows; row++) {
			...
			for (col = 1; col <= nb_columns; col++) {
				html += '<div class="cell" col="' + col + '"><div class="inner-cell">' + col + row + '</div></div>';
			}
	...
	this.unselect = function(that) {
		...
		if (that.$selectedCell) {
			that.$selectedCell.toggleClass('inner-cell', true);
			that.$selectedCell.toggleClass('inner-cell-edit', false);
			that.$selectedCell.toggleClass('inner-cell-select', false);
		...
		if (that.$editCell) {
			that.$editCell.toggleClass('inner-cell', true);
			that.$editCell.toggleClass('inner-cell-edit', false);
			that.$editCell.toggleClass('inner-cell-select', false);
			that.$editCell.parent().css('zIndex', '1');
			that.$editCell.parent().parent().css('zIndex', '1');
			// restore the value as text element
			var cellVal = that.$editCell.children().val();
			that.$editCell.children().remove();
			that.$editCell.text(cellVal);
			that.$editCell = null;
		}
		...
		if ($innerCell.length > 0) {
			if (event.type == 'click') { // only type of events for now
				...
				if (event.target.className == 'inner-cell') {
					...
					$innerCell.toggleClass('inner-cell', false);
					$innerCell.toggleClass('inner-cell-edit', false);
					$innerCell.toggleClass('inner-cell-select', true);
					$innerCell.parent().css('zIndex', '5');
					$innerCell.parent().parent().css('zIndex', '5');
					$innerCell.children().focus(); // set the focus on the selected cell
				...
				// edit the new selected cell
				$innerCell.toggleClass('inner-cell', false);
				$innerCell.toggleClass('inner-cell-select', false);
				$innerCell.toggleClass('inner-cell-edit', true);
				$innerCell.parent().css('zIndex', '5');
				$innerCell.parent().parent().css('zIndex', '5');
				
				// extract and remove the text value
				var cellVal = $innerCell.text();
				$innerCell.text('');
				
				// create an input box
				$innerCell.append('<input type="text" class="input-area" value="'+ cellVal + '">');
				$innerCell.children().focus(); // set the focus on the edit box
				
				that.$editCell = $innerCell;
				that.selectedRowNumber = parseInt($innerCell.parent().parent().attr('row'));
				that.selectedColNumber = parseInt($innerCell.parent().attr('col'));
			}
		}
		...
		if (that.$selectedCell !== null) {
			...
			// user presses the left arrow
				case 37:	if (that.selectedColNumber > 1) {							
								...
								that.adjustScrollPosition(that.selectedRowNumber, that.selectedColNumber);
							}
							break;	
				...
				case 27:
							...
							that.unselect(that);
							break;
				// for anything else we can assume the user is trying to edit the cell
				default:	
					that.$grid.$container.children().eq((that.selectedRowNumber-1)*2).children().eq(that.selectedColNumber-1).children().dblclick();
					return true; // let's make sure we still capture the key stroke
			}

		} else if (that.$editCell !== null) { //user is editing a cell, let the event bubble up, let's catch a few 
			switch(event.keyCode)
			{
				// user pressed enter as he is editing the cell
				case 13:
					that.unselect(that);
					that.$grid.$container.children().eq((that.selectedRowNumber-1)*2).children().eq(that.selectedColNumber-1).children().click();
					that.adjustScrollPosition(that.selectedRowNumber, that.selectedColNumber);
				// user pressed tab
				case 9:
					break;
	...
	this.adjustScrollPosition = function (rowNumber,colNumber) {
		...
		// right side beyond right part of the screen
		...
		// left side beyond left part of the screen
		if ((colNumber-1)*(104 + 2*1 + 2*4) <= $grid.$container.parent().scrollLeft()) {
			//$grid.$container.parent().scrollLeft((colNumber-3)*(104 + 2*1 + 2*4)); // allows 3 col on the left
			var targetOffset = (colNumber-5)*(104 + 2*1 + 2*4);
			$grid.$container.parent().animate({scrollLeft: targetOffset}, 400);
		}
		
		// top side beyond top of the screen
		if ((rowNumber-1)*(28) <= $grid.$container.parent().scrollTop()) {
			//$grid.$container.parent().scrollLeft((colNumber-3)*(104 + 2*1 + 2*4)); // allows 3 col on the left
			var targetOffset = (rowNumber-5)*(28);
			$grid.$container.parent().animate({scrollTop: targetOffset}, 300);
		}

		// bottom side beyond bottom of the screen
		if ((rowNumber)*(28) >= $grid.$container.parent().scrollTop() + 500) {
			//$grid.$container.parent().scrollLeft((colNumber-3)*(104 + 2*1 + 2*4)); // allows 3 col on the left
			var targetOffset = (rowNumber-13)*(28);
			$grid.$container.parent().animate({scrollTop: targetOffset}, 300);
		}
	}
